...
from stattest.loggers.buffering_handler import FTBufferingHandler
from stattest.loggers.rich_console import get_rich_console
from stattest.loggers.rich_handler import FtRichHandler
...
FT_LOGGING_CONFIG = {
    ...
    "handlers": {
        "console": {
            "class": "stattest.loggers.rich_handler.FtRichHandler",
            "formatter": "basic",
        },
    },
    "root": {
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "stattest": logging.INFO if verbosity <= 1 else logging.DEBUG,
...
def _create_log_config(config: Config) -> dict[str, Any]:
        ...
        if s[0] == "syslog":
            ...
            try:
                ...
                raise OperationalException(
                    ...
                    "in order to use logging to journald."
                )
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "stattest.loggers.rich_handler.FtRichHandler":
