"""Tests for the time complexity execution step context container."""
...
from pysatl_experiment.experiment_execution.step.execution_step.time_complexity import (
    time_complexity_execution_step_context,
)
from pysatl_experiment.experiment_execution.step.generation_step.generation_step_context import GenerationData


TimeComplexityExecutionStepContext = time_complexity_execution_step_context.TimeComplexityExecutionStepContext
...
def make_context(**overrides: object) -> TimeComplexityExecutionStepContext:
    """Build a fully populated context, overriding selected fields."""
    values: dict[str, object] = {
        "data_list": [make_generation_data(10), make_generation_data(20)],
        "experiment_name": "tc_experiment",
    }
    values.update(overrides)

    return TimeComplexityExecutionStepContext(**values)  # type: ignore[arg-type]


# Checks that the container is a dataclass.
def test_container_is_a_dataclass() -> None:
    assert is_dataclass(TimeComplexityExecutionStepContext)
...
def test_declares_expected_fields() -> None:
    assert {field.name for field in fields(TimeComplexityExecutionStepContext)} == {"data_list", "experiment_name"}
    assert get_type_hints(TimeComplexityExecutionStepContext) == {
...
def test_positional_and_keyword_construction_are_equivalent() -> None:
    ...
    positional = TimeComplexityExecutionStepContext(data_list, "tc_experiment")
...
@pytest.mark.parametrize("other_name", ["", "OTHER", "tc_experiment "])
def test_inequality_for_different_experiment_name(other_name: str) -> None:
    assert make_context(experiment_name="tc_experiment") != make_context(experiment_name=other_name)
...
def test_omitting_required_field_raises_type_error(missing_field: str) -> None:
    values: dict[str, object] = {"data_list": [], "experiment_name": "tc_experiment"}
    values.pop(missing_field)

    with pytest.raises(TypeError):
        TimeComplexityExecutionStepContext(**values)  # type: ignore[arg-type]


# Checks that omitting the data list entirely raises a TypeError.
def test_data_list_has_no_default() -> None:
    with pytest.raises(TypeError):
        TimeComplexityExecutionStepContext(experiment_name="tc_experiment")  # type: ignore[call-arg]


# Checks that the fields can be reassigned after construction.
...
def test_repr_contains_class_name_and_values() -> None:
    representation = repr(make_context(experiment_name="tc_experiment"))

    assert representation.startswith("TimeComplexityExecutionStepContext(")
    assert "tc_experiment" in representation
...
def test_asdict_returns_plain_field_mapping() -> None:
    context = make_context(data_list=[], experiment_name="tc_experiment")

    assert asdict(context) == {"data_list": [], "experiment_name": "tc_experiment"}
